import { einsteinAI } from "@/data/salesforce-features";

export function EinsteinAISection() {
    const Icon = einsteinAI.icon;

    return (
        <section className="relative py-20 overflow-hidden bg-muted/30">
            {/* Background decorations */}
            <div className="absolute bottom-20 right-0 w-100 h-100 bg-amber-500/10 rounded-full blur-[100px]" />
            ...
            <div className="container relative z-10 mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {einsteinAI.features.map((feature, index) => {
                        const FeatureIcon = feature.icon;
                        return (
                            <div
                                key={index}
                                className="group relative p-6 rounded-2xl bg-card/50 backdrop-blur-sm border border-amber-500/10 hover:border-amber-500/30 hover:-translate-y-1 transition-all duration-300 hover:shadow-lg hover:shadow-amber-500/5"
                            >
